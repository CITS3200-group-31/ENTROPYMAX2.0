...
class CLIIntegration:
    ...
    def validate_inputs(self, input_csv: str, gps_csv: str) -> Tuple[bool, str]:
        """
        Validate input files before running CLI
        ...
        if not Path(gps_csv).exists():
            return False, f"GPS CSV not found: {gps_csv}"
            
        # Check CLI binary exists
        if not self.cli_path.exists():
            return False, f"CLI binary not found: {self.cli_path}"
            
        # Check CLI binary is executable
        if not os.access(self.cli_path, os.X_OK):
            return False, f"CLI binary not executable: {self.cli_path}"
            
        return True, ""
        
    def get_cli_version(self) -> Optional[str]:
        """Get CLI version (if supported)"""
        try:
            result = subprocess.run(
